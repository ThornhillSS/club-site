...
// Needs testing on mobile (especially the menu component)
const Menu = () => {
    const { navigation, club, callToAction } = config;
    const { name: clubName, logo } = club;
                ...
                <div className={'relative pt-6 px-4 sm:px-6 lg:px-8'}>
                    <nav className={'relative flex items-center justify-between sm:h-10 lg:justify-start'} aria-label='Global'>
                        <div className={'flex items-center flex-grow flex-shrink-0 lg:flex-grow-0'}>
                            <div className={'flex items-center justify-between w-full md:w-auto'}>
                                <a href='#'>
                                    ...
                                    <img alt='logo' className={'h-16 w-auto sm:h-16 rounded-2xl overflow-hidden'} src={logo} />
                                </a>
                        ...
                        <div className={'hidden md:block md:ml-10 md:pr-4 md:space-x-8'}>
                            {navigation.map((item) => (
                                <Link spy={true} activeClass='active' smooth={true} duration={1000} key={item.name} to={item.href} className={'font-medium text-gray-500 hover:text-gray-900'}>
                                    {item.name}
                                </Link>
                            ))}
                            <a href='https://tssoj.ca/discord' className={'font-medium text-sky-500 hover:text-secondary'} target='_blank' rel='noreferrer'>
                                Get Started
                            </a>
                        </div>
                    </nav>
                    ...
                    <Popover.Panel focus className={'absolute z-10 top-0 inset-x-0 p-2 transition transform origin-top-right md:hidden'}>
                        <div className={'rounded-lg shadow-md bg-background ring-1 ring-black ring-opacity-5 overflow-hidden'}>
                            <div className={'px-5 pt-4 flex items-center justify-between'}>
                                <div>
                                    <img className={'h-8 w-auto rounded-2xl overflow-hidden'} src={logo} alt='' />
                                </div>
                                <div className={'-mr-2'}>
                                    <Popover.Button className={'bg-background rounded-md p-2 inline-flex items-center justify-center text-gray-400 hover:text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-secondary'}>
                            ...
                            <div className={'px-2 pt-2 pb-3 space-y-1'}>
                                {navigation.map((item) => (
                                    <Link spy={true} activeClass='active' smooth={true} duration={1000} key={item.name} to={item.href} className={'block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-gray-900 hover:bg-gray-50'}>
                                        {item.name}
                                    </Link>
                                ))}
                            </div>
                            <a href={callToAction.href} className={'block w-full px-5 py-3 text-center font-medium text-sky-500 bg-gray-50 hover:bg-gray-100'}>
                                {callToAction.text}
                            </a>
                        </div>
                    </Popover.Panel>
